...
/*	private ImageGeometry imageGeometry350x350 = new ImageGeometry(new ImageSize(350, 350), GeometryOperator.Emphasize);
	...
	@RequestMapping(method = { RequestMethod.GET }, value = { "/{width:\\d+}x{hight:\\d+}/{groupId:group\\d+}/{f1:M\\d+}/{f2:\\w{2}}/{f3:\\w{2}}/{f4:.*}" } )
	@ResponseBody
	public byte[] show160x160Image(
			@PathVariable("width") Integer width,@PathVariable("hight") Integer hight,
			@PathVariable("groupId") String groupId, @PathVariable("f1") String f1, @PathVariable("f2") String f2,
			@PathVariable("f3") String f3, @PathVariable("f4") String f4, HttpServletRequest request, HttpServletResponse response) throws NoSuchRequestHandlingMethodException {
		try {
			ImageGeometry imageGeometry = new ImageGeometry(new ImageSize(width,hight), GeometryOperator.Emphasize);
			String resourcePath = new StringBuilder().append(groupId).append("/").append(f1).append("/").append(f2).append("/").append(f3).append("/").append(f4).toString();
			byte[] result = resourceService.getResourceByPath(resourcePath);
			if (ArrayUtils.isEmpty(result)) {
				throw new NoSuchRequestHandlingMethodException(request);
			}
			result = imageService.resizeImage(resourcePath, result, imageGeometry);
			browserCacheGenerator.checkAndPrepare(request, response, expireSeconds, true);
			return result;
		} catch (Exception e) {
			throw new NoSuchRequestHandlingMethodException(request);
	...
	@ResponseBody
	public Object sss(HttpServletRequest request, HttpServletResponse response) throws NoSuchRequestHandlingMethodException {
		return "sdfdfdf";
	}
}
